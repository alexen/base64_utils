...
TEST( Custom, EncodeEmptyString )
{
     aux::inmem_streambuf sbuf{ "", 0 };
     std::istream is{ &sbuf };
     std::ostringstream os;

     ASSERT_NO_THROW( base64::custom::encode( is, os ) );
     ASSERT_TRUE( os.str().empty() );
}


TEST( Custom, EncodeTextString )
{
...
TEST( Custom, EncodeBinaryString )
{
...
TEST( Custom, DecodeTextString )
{
     ...
     ASSERT_NO_THROW( base64::custom::decode( is, os ) );
     ASSERT_FALSE( os.str().empty() );
     ASSERT_EQ( os.str(), expected );
}


TEST( Custom, DecodeBinaryString )
{
     const auto source =
          "sGhzHnXAsTc+H/WktEaKLBwpUzEgvlWG2S38fGUVrKrY1l6LgZyda1k"
          "pFoteKd9jzuW7wdc55sf93fj1Disu209opz4eLLtcdCf3iHq1rAtlFf"
     ...
     ASSERT_NO_THROW( base64::custom::decode( is, os ) );
     ASSERT_FALSE( os.str().empty() );
     ASSERT_EQ( n_binaries, os.str().size() );
     ASSERT_EQ( 0, std::memcmp( binaries, os.str().data(), n_binaries ) );
}
